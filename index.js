...
class ParadoxTable {
  constructor(buffer) {
    ...
    this.primaryIndexWorkspace = new pointer(
      buffer,
      this.indexFieldNumber.upperlimmit,
    );
    ...
    this.cryptInfoEndPtr = new pointer(
      buffer,
      this.cryptInfoStartPtr.upperlimmit,
    );
    ...
    this.sometimesNumFields = new integer(
      buffer,
      this.hiFieldIDinfo.upperlimmit,
    );

    this.dosGlobalCodePage = new integer(
      buffer,
      this.sometimesNumFields.upperlimmit,
    );
    ...
    for (
      var i = parseInt(78, 16);
      i < parseInt(78, 16) + this.numFields.getValue() * 2;
      i += 2
    ) {
    ...
    var fieldsString = buffer
      .slice(initialBlockLimit, blockLimit)
      .toString("utf8");
    ...
    this.firstaddDataSize = this.buffer.slice(
      this.headerSize.getValue() + 4,
      this.headerSize.getValue() + 6,
    );
  ...
  findRecords(options = {}) {
    ...
    for (var i = 0; i < numberOfBlocks; i++) {
      var addDataSize = getAddDataSize(
        this.buffer,
        this.headerSize.getValue() + blockSize * i,
      );
      //console.log( (this.headerSize.getValue() + blockSize * i).toString(16))
      var numRecordsInBlock =
        addDataSize.readUInt16LE() / this.recordSize.getValue() + 1;
      ...
      for (var j = 0; j < numRecordsInBlock; j++) {
        ...
        for (var k = 0; k < this.TFldInfoRecArray.length; k++) {
          record[this.TFldInfoRecArray[k].name] = new Field(
            this.TFldInfoRecArray[k].name,
